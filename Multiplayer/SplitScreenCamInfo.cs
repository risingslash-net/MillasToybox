...
namespace RisingSlash.FP2Mods.MillasToybox
{
    public class SplitScreenCamInfo
    {
        ...
        public SplitScreenCamInfo(FPCamera newFpCamScript, GameObject goRenderCamera)
        {
            ...
            {
                MillasToybox.LogWarning("Null render cam in split screen info, try fetching later.");
            }
        }
        ...
        {
            //MillasToybox.Log($"Was passed playerIndex: {playerIndex} playerNum: {playerNum}");
            if (playerNum <= 1)
            {
                return SingleFull;
            }
